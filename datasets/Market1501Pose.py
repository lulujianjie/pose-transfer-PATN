...
class Market1501Pose():
    def __init__(self, data_dir = 'data_dir', verbose = True, split = 'train', restore = 'True'):
        ...
        if self.restore:
            self.datalist = np.load(self.dataset_dir+'/pose/{}.npy'.format(self.split))
            if verbose:
    ...
    def get_path_list(self, data_dir, split):
        ...
        if split == 'train':
            ...
            for i in range(len(dataset_pair)):
                ...
                pose_heatmap_path1 = data_dir + 'train_pose_heatmap/' + dataset_pair.iloc[i]['from']+'.npy'
                pose_heatmap_path2 = data_dir + 'train_pose_heatmap/' + dataset_pair.iloc[i]['to'] + '.npy'
            ...
            for i in range(len(dataset_pair)):
                ...
                pose_heatmap_path1 = data_dir + 'test_pose_heatmap/' + dataset_pair.iloc[i]['from']+'.npy'
                pose_heatmap_path2 = data_dir + 'test_pose_heatmap/' + dataset_pair.iloc[i]['to'] + '.npy'
                dataset.append((img_path1, pose_heatmap_path1, img_path2, pose_heatmap_path2))
        return dataset
