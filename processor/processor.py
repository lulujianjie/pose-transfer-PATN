...
def do_train(
    ...
    logger = logging.getLogger('pose-transfer-avs.train')
    ...
    for epoch in range(1, epochs+1):
        ...
        for iter, batch in enumerate(train_loader):
            ...
            input_G = (img1,torch.cat((pose1,pose2), 1))
            ...
            loss_G = 0.5*loss_G_ii*Cfg.LOSS.GAN_WEIGHT + 0.5*loss_G_ip*Cfg.LOSS.GAN_WEIGHT+loss_L1
            loss_G.backward()
            optimizerG.step()
...
def do_inference(Cfg,
    ...
    output_dir = Cfg.DATALOADER.LOG_DIR
    device = "cuda"
    logger = logging.getLogger("pose-transfer-avs.test")
    ...
    for i, batch in enumerate(tmp_results):
        for idx in range(batch.shape[0]):
            ...
            cv2.imwrite('./log/img2/{}_{}.png'.format(i,idx), cv2.cvtColor(img2, cv2.COLOR_RGB2BGR))
            cv2.imwrite('./log/fake_img2/{}_{}.png'.format(i,idx), cv2.cvtColor(fake_img2, cv2.COLOR_RGB2BGR))
